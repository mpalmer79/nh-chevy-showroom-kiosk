...
The NH Chevy Showroom Kiosk is a production-ready monorepo powering New Hampshire Chevrolet' in-store customer experience. The system is designed with the understanding that **the customer is already in the showroom** — the AI assistant acts as a knowledgeable salesperson, not a website chatbot.
...
- **Slack Integration**: Real-time notifications to `#showroom-kiosk-alerts` when customers request assistance
...
EMAIL_FROM_ADDRESS=kiosk@example.com
EMAIL_FROM_NAME=NH Chevy Showroom Kiosk
EMAIL_NOTIFY_SALES=sales@example.com
EMAIL_NOTIFY_DEFAULT=manager@example.com
...
|-------------|------------|-------|--------------|
| **Production** | [/docs](REPLACE_ME_BACKEND_URL/docs) | [/redoc](REPLACE_ME_BACKEND_URL/redoc) | [/openapi.json](REPLACE_ME_BACKEND_URL/openapi.json) |
...
**Production URLs:**
- Frontend: REPLACE_ME_FRONTEND_URL
- Backend: REPLACE_ME_BACKEND_URL
...
```bash
# Build images
docker build -t showroom-kiosk-backend ./backend
docker build -t showroom-kiosk-frontend ./frontend

# Run containers
docker run -p 8000:8000 -e ANTHROPIC_API_KEY=xxx showroom-kiosk-backend
docker run -p 3000:3000 showroom-kiosk-frontend
```
...
- **GitHub Issues**: [Create an issue](https://github.com/mpalmer79/nh-chevy-showroom-kiosk/issues)
- **Dealership IT Team**: Contact internal IT support

---

*Built for a New Hampshire Chevrolet dealership*
